...
import { ArrowLeft, Check, Download, ImagePlus, Loader2, LockKeyhole, Play, Sparkles, Square, Trash2, UploadCloud, X } from "lucide-react";
...
export default function BackgroundRemover() {
  ...
  return <div className="remover-shell">
    <header className="remover-header"><button onClick={() => navigate("/studio")}><ArrowLeft size={16} /> Studio</button><a href="/" className="remover-brand"><span><Sparkles size={14} /></span> Sweet AI Lab by SONET</a><div><span><LockKeyhole size={13} /> {creditCount === null ? "Checking credits" : `${creditCount} credits`}</span><button onClick={() => navigate("/studio")}>AI-Powered Tools for Creators</button><AppThemeToggle /></div></header>
    <main className="remover-main">
      <section className="remover-intro"><div className="remover-kicker"><Sparkles size={13} /> Browser-local workflow</div><h1>Remove backgrounds,<br /><em>keep control.</em></h1><p>RMBG-1.4 runs in a dedicated browser worker through ONNX/WASM. Your image stays on this device while it is processed.</p></section>
