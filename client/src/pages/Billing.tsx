...
export default function Billing() {
  ...
  const receiver = (import.meta.env.VITE_BKASH_RECEIVER as string | undefined) || "01797953059";
  const whatsapp = (import.meta.env.VITE_WHATSAPP_SUPPORT_NUMBER as string | undefined) || "01797953059";

  useEffect(() => {
    void supabase.auth.getSession().then(({ data }) => {
      if (!data.session) navigate("/");
      else setEmail(data.session.user.email ?? "");
    }).catch(() => setError("We could not check your sign-in. Please refresh and try again."));
  }, [navigate]);
  ...
  async function submitPayment(event: FormEvent) {
    ...
    try {
      ...
      setError("We could not save the request. Please refresh and try again.");
    } finally {
      setSubmitting(false);
    }
  }
...
function toWhatsAppId(value?: string) {
  const digits = (value ?? "01797953059").replace(/\D/g, "");
