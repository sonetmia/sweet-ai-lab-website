...
import { ArrowRight, Check, FileText, ImageIcon, Loader2, LockKeyhole, Orbit, ShieldCheck, Sparkles, WandSparkles } from "lucide-react";
...
const tools = [
  ...
  { icon: WandSparkles, eyebrow: "Image-to-Prompt", title: "Turn a visual language into a reusable prompt.", copy: "Transform source images into detailed, style-aware prompts for your next generation workflow.", tint: "violet" },
  ...
  return (
    <div className="site-shell">
      ...
      <header className="landing-header container">
        <a className="brand" href="/" aria-label="Sweet AI Lab by SONET home"><span className="brand-mark"><Orbit size={18} strokeWidth={2.4} /></span><span>Sweet AI Lab <em>by SONET</em></span></a>
      ...
      <main>
        <section className="hero container">
          <div className="hero-copy">
            <div className="eyebrow"><Sparkles size={13} /> Built for meticulous stock creators</div>
          ...
          <div className="hero-art" aria-label="Product workflow preview">
            <div className="art-topline"><span className="signal-dot" /> Studio workspace <span>Live system</span></div>
            <div className="art-stage"><div className="art-orbit orbit-a" /><div className="art-orbit orbit-b" /><div className="floating-card card-metadata"><div className="floating-icon mint"><FileText size={19} /></div><div><small>METADATA</small><strong>Platform-aware</strong></div><Check size={17} className="card-check" /></div><div className="central-graphic"><Sparkles size={41} /><span>AI</span></div><div className="floating-card card-prompt"><div className="floating-icon violet"><WandSparkles size={18} /></div><div><small>PROMPT</small><strong>Style mapped</strong></div></div><div className="floating-card card-local"><div className="floating-icon amber"><LockKeyhole size={18} /></div><div><small>PROCESSING</small><strong>Browser-local</strong></div></div></div>
        ...
        <section className="workflow-section" id="workflow"><div className="container workflow-grid"><div className="workflow-copy"><div className="eyebrow">A deliberate studio routine</div><h2>Less tab switching.<br /><em>More considered output.</em></h2><p>Upload once, choose a target platform, calibrate your output, and export directly into the format you need. The dashboard is designed to keep your decisions close at hand.</p><div className="workflow-points"><span><Check size={15} /> Platform-specific CSV export</span><span><Check size={15} /> Own-key and paid AI paths</span><span><Check size={15} /> Credit history you can inspect</span></div></div><div className="workflow-panel"><div className="panel-heading"><span>CREATIVE QUEUE</span><b>03 ready</b></div>{["summer-garden-illustration.ai", "studio-ceramics-set.jpg", "coastal-poster-series.png"].map((file, index) => <div className="queue-row" key={file}><span className={`queue-index q-${index + 1}`}>0{index + 1}</span><div><strong>{file}</strong><small>{index === 0 ? "Metadata ready" : index === 1 ? "Prompt prepared" : "Ready to process"}</small></div><span className={index === 0 ? "status-done" : "status-ready"}>{index === 0 ? "Complete" : "In queue"}</span></div>)}<div className="panel-bottom"><span><span className="credit-pulse" /> 10 credits available</span><button>Open studio <ArrowRight size={14} /></button></div></div></div></section>
        ...
        <section className="closing-section container"><div><Sparkles size={23} /><h2>A cleaner space for<br /><em>your next submission.</em></h2></div><button className="primary-button" onClick={continueWithGoogle}>Start with Sweet AI Lab <ArrowRight size={17} /></button></section>
      </main>
      <footer className="landing-footer container"><span>© {new Date().getFullYear()} Sweet AI Lab by SONET</span><span>AI-Powered Tools for Creators</span><div><a href="#tools">Tools</a><a href="#plans">Plans</a><a href="mailto:hello@sweetailab.com">Contact</a></div></footer>
    </div>
  );
}
